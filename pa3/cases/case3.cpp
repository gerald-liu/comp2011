...
bool ProcessNextFrame_(Video & video)
{
	...
	if (new_frame_index != 0)
	{
		...
		for (int v_idx = 0; v_idx < MAX_VEHICLE_NUM; ++v_idx)
		{
			...
			/**************** test1 start *****************/
			VehicleFrameInfo * test_vf_info1 = GetVFInfo(vehicle, new_frame_index);
			if (test_vf_info1 == nullptr)
			{
				cout << "should be here\n";
			}
			else
			...
			if (new_vf_info != nullptr)
			{
				...
				/**************** test2 start *****************/
				VehicleFrameInfo * test_vf_info2 = GetVFInfo(vehicle, new_frame_index);
				if (test_vf_info2 == nullptr)
				{
					cout << "should not be here\n";
				}
				else
				{
					cout << "should be here vf_info: " << test_vf_info2->frame_index << " " 
						<< test_vf_info2->vehicle_index << " "
						<< test_vf_info2->position[0] << " " << test_vf_info2->position[1] << " " 
						<< test_vf_info2->speed << "\n";
				}
				/***************** test2 end ******************/
			}
