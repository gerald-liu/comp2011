 ...
 *
 * This is the solution code for reference.
 * 
 */
...
// Check whether (x, y) is a valid position to mark
bool inputWithinBound(int x, int y, int lower_x, int upper_x, int lower_y, int upper_y){
...
bool isInputValid(int x, int y, int lower_x, int upper_x, int lower_y, int upper_y, const int board[][9]){
    ...
    // Please fill in your codes here.
    if (!inputWithinBound(x, y, lower_x, upper_x, lower_y, upper_y)){
        cout << "Position out of bound! ";
    }else if (board[x][y] != 0){
        cout << "Position has been dominated! ";
    }else{ return true;}
    return false;
}
